...
public class MainFrame extends JFrame {
    ...
    matchTask.setProtocol(new AbstractGameProtocol() {
      ...
      private int clickMatch(Pixel p) throws RobotInterruptedException, IOException, AWTException {
        ...
        if (duelsFull) {
          ...
          int min0 = settings.getInt("tasks.matches.min0Diff", mind);
          int min1 = settings.getInt("tasks.matches.min1Diff", min0);
          int min2 = settings.getInt("tasks.matches.min2Diff", min1);

          int maxd = settings.getInt("tasks.matches.maxDiff", 400);

          if (attrs.get(0).deviation() <= 25) {
            int diff = myAttrs.sum() - attrs.get(0).sum();
            attrs.get(0).diff = diff;
            if (diff > min0 && diff < maxd) {
              attrs.get(0).good = true;
            }
          }

          if (attrs.get(1).deviation() <= 25) {
            int diff = myAttrs.sum() - attrs.get(1).sum();
            attrs.get(1).diff = diff;
            if (diff > min1 && diff < maxd) {
              attrs.get(1).good = true;
            }
          }

          if (attrs.get(2).deviation() <= 25) {
            int diff = myAttrs.sum() - attrs.get(2).sum();
            attrs.get(2).diff = diff;
            if (diff > min2 && diff < maxd) {
              attrs.get(2).good = true;
            }
          }
          LOGGER.info("A1: " + attrs.get(0) + "  " + attrs.get(0).diff + "  " + attrs.get(0).good);
          LOGGER.info("A2: " + attrs.get(1) + "  " + attrs.get(1).diff + "  " + attrs.get(1).good);
          LOGGER.info("A3: " + attrs.get(2) + "  " + attrs.get(2).diff + "  " + attrs.get(2).good);

          String[] order = settings.getProperty("tasks.matches.order", "2,1,0").split(",");
          for (int i = 0; i < order.length; i++) {
            int sl = Integer.parseInt(order[i]);
            if (attrs.get(sl).good) {
              slot = sl;
              break;
            }
          }
  ...
  private final class PairsProtocol extends AbstractGameProtocol {
           ...
           // mouse.delay(4000);
            ...
            if (p != null) {
              ...
              if (pq != null) {
                ...
                mouse.delay(4000);
                if (!doPairs(settings.getProperty("pairs.image", "lib/back2.bmp"), false, PairsLayout.NONE)) {
                  LOGGER.info("no energy");
                  LOGGER.info("sleep 5min");
                  sleep(5 * 60000);
                  handlePopups();
    ...
    public boolean doPairs(String imageName, boolean fast, PairsLayout pairsLayout) throws RobotInterruptedException,
      ...
      do {
        ...
        if (p != null) {
          ...
          int slotsNumber = 0;
          do {
            slotsNumber = workPairs(pp, imageName, fast, pairsLayout);
          } while (slotsNumber > 0);
    ...
    private int workPairs(final Pixel pp, String imageName, boolean fast, PairsLayout pairsLayout) {
      ...
      Rectangle gameArea = new Rectangle(pp.x + 119, pp.y + 123, mwidth, mheight);
      ...
      try {
        ...
        if (pairsLayout == PairsLayout.NONE) {
            ...
            for (int col2 = 1; col2 <= mcols; col2++) {
              ...
              if (c1.equals(c2)) {
                ...
                if (maxClicks < 0 || clicks < maxClicks) {
                  ...
                  Slot slot1 = matrix.get(c1);
                  Slot slot2 = matrix.get(c2);
                  if (slot1 != null && slot1.active && slot2 != null && slot2.active
                      && sameImage(slot1.image, slot2.image)) {
                    clicks++;
      ...
      AbstractAction action = new AbstractAction("Pairs") {
        public void actionPerformed(ActionEvent e) {
          Thread t = new Thread(new Runnable() {
            public void run() {
              try {
                new PairsProtocol().doPairs(settings.getProperty("pairs.image", "lib/back2.bmp"), false,
                    PairsLayout.NONE);
              } catch (RobotInterruptedException e) {
                LOGGER.info("INTERRUPTED");
              } catch (IOException e) {
  ...
  @SuppressWarnings("serial")
  private JToolBar createToolbar1p() {
    JToolBar mainToolbar1 = new JToolBar();
    mainToolbar1.setFloatable(false);
